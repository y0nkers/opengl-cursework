...
glm::mat4 Camera::GetProjectionMatrix() { return glm::perspective(glm::radians(Fov), AspectRatio, zNear, zFar); }
...
void Camera::Move(int32_t dirs, float deltaTime)
{
	...
	if (direction != glm::vec3(0, 0, 0)) direction = glm::normalize(direction);

	Position += Front * velocity * direction.z;
	Position += Right * velocity * direction.x;
	Position += Up * velocity * direction.y;
	UpdateCameraVectors();
	if (!noClip) Position.y = 0.0f;
}

void Camera::Rotate(float xoffset, float yoffset, GLboolean constrainPitch)
{
	xoffset *= Sensitivity; // Cмещение по OX
	yoffset *= Sensitivity; // Cмещение по OY

	Yaw += xoffset;
	Pitch += yoffset;

	if (constrainPitch) // Ограничение для наклона вперёд/назад
	{
