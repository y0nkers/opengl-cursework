...
void Model::Draw(Program& program, bool isPickingPhase)
{
	for (unsigned int i = 0; i < meshes.size(); i++)
		meshes[i].Draw(program, isPickingPhase);
}
...
Mesh Model::processMesh(aiMesh* mesh, const aiScene* scene)
{
	...
	{
		aiFace face = mesh->mFaces[i]; // сетка как массив граней
		for (unsigned int j = 0; j < face.mNumIndices; j++) // грань содержит список индексов вершин
...
// Проходим по всем текстурам указанного типа, считываем пути к файлам, загружаем и генерируем текстуры
